...
(async () => {
  ...
  process.chdir(tmp); process.env.QIQ_ALLOW_SYNTHETIC = "1";   // this script alone may normalise answer-key data, and only from its temp dir
  const ds = buildDataset();
  for (const s of ds.summaries) console.log(`[answer-key synthetic] ${s.name.padEnd(30)} quoted ${s.lines_quoted}/30 missing ${s.lines_missing.length} review ${s.lines_needing_review.length} basis ${s.basis} discount ${s.discount ? `${s.discount.pct}% ${s.discount.applied ? "applied" : "not applied"}` : "-"} questionnaire ${s.quality.passed_mandatory ? "PASS" : "not passed (" + s.quality.failed_criteria.join("; ") + ")"}`);
