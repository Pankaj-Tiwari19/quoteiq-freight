...
async function pdfText(file: string): Promise<string> {
  ...
  // Vercel/serverless does not reliably include PDF.js's worker as an
  // implicit dependency. Load it explicitly so PDF.js can use its
  // loopback worker without falling back to a missing worker file.
  ...
  if (!globalPdfjs.pdfjsWorker?.WorkerMessageHandler) {
    globalPdfjs.pdfjsWorker = pdfWorker;
  }
...
export async function parseVendorDoc(v: Vendor): Promise<ParsedDoc> {
  ...
  if (d.kind === "text") {
    return {
      ...d,
      text: `${d.text}\n\n${att}`,
      note: `${d.note} Attachment text appended.`,
    };
  }

  return {
    ...d,
    note: `${d.note} Attachment text: ${att.slice(0, 2000)}`,
  };
}
  ...
  switch (v.response_format) {
    case "xlsx": {
      ...
      return {
        ...
        text: out.join("\n"),
        note:
          "Excel workbook flattened as 'row N: cell | cell'.",
      };
    }
    ...
    case "pdf": {
      ...
      return {
        ...
        note:
          "PDF sent inline as a document. Cite page numbers in evidence.page.",
      };
    }
    ...
    case "email": {
      ...
      return {
        kind: "text",
        text,
        note: "Raw email (.eml) with line numbers.",
      };
    }
      ...
      return {
        ...
        media_type:
          path.extname(file).toLowerCase() === ".png"
            ...
            : "image/jpeg",
        note:
          "Photograph of a printed rate card; read visually. Smudged or unreadable digits must come back as null with a low confidence, never guessed.",
      };
    }
  }
}